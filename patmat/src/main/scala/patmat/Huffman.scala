...
object Huffman {
   ...
   *   }
   */
    def times(chars: List[Char]): List[(Char, Int)] = chars.map(i => (i,chars.count(_ ==i))).toSet.toList
   ...
   */
    def combine(trees: List[CodeTree]): List[CodeTree] = {
      if (singleton(trees)) trees
        ...
        case left :: right :: xs => (makeCodeTree(left, right) :: xs).sortWith((x1,x2) => weight(x1) < weight(x2))
        case left :: right :: Nil => List(makeCodeTree(left,right))
      }
    }
   ...
   */
    def until(checkSingle: List[CodeTree] => Boolean, mergeTree: List[CodeTree] => List[CodeTree])(tree: List[CodeTree]): List[CodeTree] ={
      if (checkSingle(tree)) tree
      else until(checkSingle, mergeTree)(mergeTree(tree))
    }
   ...
   */
    def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      def lookup(t: CodeTree)(c:Char): List[Bit] = t match {
        case Leaf(ch,_) => List()
        case Fork(l,r,ch,w) if chars(l).contains(c) => 0 :: lookup(l)(c)
        case Fork(l,r,_,_) => 1 :: lookup(r)(c)
      }
