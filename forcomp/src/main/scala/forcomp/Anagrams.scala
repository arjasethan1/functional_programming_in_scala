...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase.groupBy(x => x).mapValues(_.size).toList.sortBy(_._1)

  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences = s.flatMap(x => wordOccurrences(x)).groupBy(_._1).mapValues(_.map(_._2).sum).toList.sortBy(_._1)
   ...
   */
  def combinations(occurrences: Occurrences): List[Occurrences] = (occurrences.map( occ => (1 to occ._2).map( o => (occ._1, o) ).toList )).foldLeft(List(List[(Char, Int)]()))( (l1, l2) =>
    l1 ::: (
      for {
        elem1 <- l1
        elem2 <- l2
      } yield elem1 ::: List(elem2)
      )
  )
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = y.foldLeft(x){
    (newx, elemy) =>
      for (elemx <- newx) yield (elemx._1, if(elemx._1 == elemy._1) elemx._2 - elemy._2 else elemx._2)
  }.filter(_._2 > 0)
