...
*/
  ...
  */
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    @tailrec
    def helper(i: Int, mx: Float): Float = {
      if (i < until) {
        val angle = max(input(i) / i, mx)
        helper(i + 1, angle)
      }
  ...
  def upsweep(input: Array[Float], from: Int, end: Int,
    ...
    if(end - from <= threshold) Leaf(from, end, upsweepSequential(input, from, end))
      ...
      val mid = (end - from)/ 2
      val (valL, valR) = parallel(upsweep(input, from, mid, threshold), upsweep(input, mid, end, threshold))
      Node(valL, valR)
    }
  }
  */
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float],
    ...
    var i = from
    while(i < until){
      output(i ) = max(startingAngle, input(i))
      i += 1
    }
  }
  */
  def downsweepSequential(input: Array[Float], output: Array[Float], startingAngle: Float, from: Int, until: Int)
  : Unit = if (from < until) {
