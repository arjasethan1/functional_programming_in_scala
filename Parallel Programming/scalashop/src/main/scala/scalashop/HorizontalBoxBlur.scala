...
/*
    val numTasks : List[(Int,Int)] = {
      val rangeList = (0 until src.height-1).toList
      if (rangeList.max == src.height -1) rangeList.zip(rangeList.tail)
      else (rangeList:+ (src.height -1)).zip((rangeList:+ (src.height -1)).tail)
    }
